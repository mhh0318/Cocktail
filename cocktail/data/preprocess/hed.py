...
if __name__ == '__main__':
    ...
    for file in files:
        ...
        inimg = PIL.Image.open(file).convert('RGB')
        ...
        PIL.Image.fromarray((tenOutput.clip(0.0, 1.0).numpy().transpose(1, 2, 0)[:, :, 0] * 255.0).astype(numpy.uint8)).save(os.path.join(save_path, os.path.basename(file)))
    ...
    # tenOutput = (255.0 - estimate(tenInput).clip(0.0, 1.0).numpy().transpose(1, 2, 0)[:, :, 0] * 255.0).astype(numpy.uint8)
    # PIL.Image.fromarray(tenOutput).save(save_path)
