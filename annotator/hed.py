...
def run_hed(input_path, save_path):
    inimg = PIL.Image.open(input_path).convert('RGB')
    ...
    try:
        ...
        PIL.Image.fromarray((tenOutput.clip(0.0, 1.0).numpy().transpose(1, 2, 0)[:, :, 0] * 255.0).astype(numpy.uint8)).save(save_path)
    except:
        print(f'{input_path} is wrong.')


class HEDdetect():
    def __call__(self, inimg):
        tenInput = torch.FloatTensor(numpy.ascontiguousarray(inimg[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0)))
        tenOutput = estimate(tenInput)
